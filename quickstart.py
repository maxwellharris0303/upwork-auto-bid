...
SAMPLE_RANGE_NAME = 'upwork profile!A1:D'
...
def main():
    ...
    try:
        ...
        # getProfileData()
        # columnCount1 = getColumnCount()
        # # start_index_to_insert_bid_status = columnCount + 1
        # insertBidStatusData(f'upwork profile!C{columnCount1}:C', 50)
            # print('%s, %s' % (row[3], row[4]))
        # insertData('upwork profile!A2:A', 'oscar@gmail.com', 'upwork profile!B2:B')
        # START_INDEX_TO_INSERT_PROFILE = 3
        # email = 'oscar@gmail.com'
        # insertData(f'upwork profile!A{START_INDEX_TO_INSERT_PROFILE}:A', email, f'upwork profile!B{START_INDEX_TO_INSERT_PROFILE}:B')
...
def insertJobDescription(range_name, job_description):


    values_job_description = [
        [job_description]
    ]
    request_body = {
        'values': values_job_description
    }
    ...
    try:
        ...
        print('{0} job description updated.'.format(result.get('updatedCells')))
    except HttpError as err:
        print(err)
