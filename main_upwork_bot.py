...
def main(create_account_count, campaign, searchPhase):
    SKILL_SET = [
        ...
        "List Building", "Python-Requests", "Web Crawler"
    ]
    ...
    OVERVIEW_TEXT = ["Hi, I am a developer have experience 2 years in web scraping, especially in the areas of web scraping and Flask frameworks. Please contact me. I will do my best"]
    ...
    STREET_ADDRESS = ["2198 Orenda Rd", "1313 Wyecroft Road", "2486 Albert Street", "4193 Bloor Street", "763 Burdett Avenue"]
    CITY_NAME = ["Brampton", "Burlington", "Stratford", "Vegreville", "Victoria"]
    PHONE_NUMBER = ["905-798-6537", "905-315-2377", "519-301-7797", "780-741-3097", "250-412-5930"]
    ZIP_CODE = ["L6W 1Z2", "L7P 2S9", "N5A 3K5", "T0B 4L0", "V8W IE9"]
    ...
    # FILTERED_JOB_URL = f"https://www.upwork.com/nx/jobs/search/?q=Develop%20and%20implement%20data%20extraction%20strategies%20to%20gather%20information%20from%20various%20online&sort=recency&payment_verified=1&per_page=10"
    FILTERED_JOB_URL = f"https://www.upwork.com/nx/jobs/search/?q={searchPhase}&sort=recency&per_page=10"
    ...
    while(CREATE_INDEX <= CREATE_PROFILE_COUNT):
        try:
            ...
            sleep(5000)
            driver.find_element(By.CSS_SELECTOR, "div[class=\"air3-dropdown-icon air3-icon md\"]").click()
            country_option = WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "div[class=\"air3-dropdown-menu-container\"]")))
            country_list = country_option.find_elements(By.TAG_NAME, "li")
            for country in country_list:
                if country.text == "Canada":
                    driver.execute_script("arguments[0].scrollIntoView(false);", country)
                    country.click()
                    break
            ...
            if hasResume:
                ...
                try:
                    ...
                    try:
                        birthday_input = WebDriverWait(driver, 15).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[aria-labelledby=\"date-of-birth-label\"]")))
                ...
                # sleep(1000)
                def unavailable_job_save(title):
                    with open("unavailable_jobs.txt", "a") as file:
                        file.write(title + "\n")
                ...
                file = open("unavailable_jobs.txt", "r")
 
                for line in file:
                    unavailable_job_list.append(line)
                ...
                try:    
                    job_title_list = []
                    job_title_elements = driver.find_elements(By.CSS_SELECTOR, "h2[class=\"my-0 p-sm-right job-tile-title\"]")
                    for job_title_element in job_title_elements:
                        job_title_list.append(job_title_element.text)

                    available_job_title = find_first_non_equal(unavailable_job_list, job_title_list)
                    for job_title_element in job_title_elements:
                        if available_job_title == job_title_element.text:
                            JOB_TITLE = available_job_title
                    ...
                    open_new_window = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-test=\"slider-open-in-new-window\"]")))
                    ...
                    # driver.execute_script(f'document.querySelectorAll(\'h2[class="h5 mb-0 mr-2"]\')[0].click()')
                    job_title_list = []
                    job_title_elements = driver.find_elements(By.CSS_SELECTOR, "h2[class=\"h5 mb-0 mr-2 job-tile-title\"]")
                    for job_title_element in job_title_elements:
                        job_title_list.append(job_title_element.text)
                    ...
                    for job_title_element in job_title_elements:
                        if available_job_title == job_title_element.text:
                            ...
                            break
                    sleep(3)
                    open_new_window = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "a[data-test=\"slider-open-in-new-window UpLink\"]")))
                    new_url = open_new_window.get_attribute('href')
                ...
                try:
                    verify_text = driver.find_element(By.CSS_SELECTOR, "div[class=\"air3-modal-footer air3-btn-row air3-btn-row-right\"]").text
                    if "Verify your identity" in verify_text:
                        driver.quit()
                        unavailable_job_save(JOB_TITLE)
                        continue
                except:
                    pass
                ...
                try:
                    ...
                    for i in textarea_label:
                        if i.text != "Cover Letter" and i.text != "Attachments":
                ...
                response_chatgpt_array = bid_function(job_desc_text, array_extra_questions)
                ...
                quickstart.insertJobDescription(f'upwork profile!F{columnCount1}:F', job_desc_text)
