...
SKILL_SET = ["Full-stack Development", "Front-End Development", "Angular", "Vue.js", "Back-End Development", "Laravel", "Firebase Cloud Firestore", "Tailwind CSS", "HTML5", "CSS 3", "Google APIs"]
...
def bid_function(job_desc, extra_questions):
    ...
    chatgpt_driver = webdriver.Chrome()
    chatgpt_driver.maximize_window()
    chatgpt_driver.get("https://chat.troioi.io/")

    new_chat_button = WebDriverWait(chatgpt_driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class=\"flex items-center w-full h-[40px] rounded-lg dark:bg-slate-1100 bg-slate-500 border border-neutral-300 text-sm dark:hover:bg-slate-800 hover:bg-slate-600\"]")))
    new_chat_button.click()

    input_chatgpt_text = WebDriverWait(chatgpt_driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "textarea[class=\"dark:text-white text-black m-0 w-full resize-none outline-none border-0 bg-transparent p-0 pr-7 focus:ring-0 focus-visible:ring-0  pl-2 md:pl-0 no-scrollbar\"]")))
    input_chatgpt_text.send_keys(BID_REQUEST)

    WebDriverWait(chatgpt_driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class=\"absolute bottom-3 right-4 focus:outline-none dark:text-white text-black\"]"))).click()

    WebDriverWait(chatgpt_driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class=\"flex items-center px-3 py-1 text-sm font-medium transition-colors rounded-full text-slate-200 bg-slate-800 hover:text-white hover:bg-slate-700\"]")))

    response_chatgpt = WebDriverWait(chatgpt_driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "div[class=\"dark:text-white text-black prose prose-invert min-w-full pr-12 mt-[-2px]\"]:nth-child(2)")))
    ...
    if extra_questions_count == 0:
        ...
        while(index_question < extra_questions_count):
            ...
            print(len(chatgpt_driver.find_elements(By.CLASS_NAME, 'dark:text-white text-black prose prose-invert min-w-full pr-12 mt-[-2px]')))
            response_chatgpt = chatgpt_driver.find_elements(By.CLASS_NAME, 'dark:text-white text-black prose prose-invert min-w-full pr-12 mt-[-2px]')[int(increase_index) + 3]
            response_chatgpt_result = response_chatgpt.find_element(By.CSS_SELECTOR, ":first-child")
...
def email_save(email):
    ...
    # start_index_to_insert_profile = columnCount + 1

    quickstart.insertData(f'upwork profile!A{columnCount + 1}:A', email, f'upwork profile!B{columnCount + 1}:B')
...
while(CREATE_INDEX <= CREATE_PROFILE_COUNT):
    try:
        ...
        WebDriverWait(driver1, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "div[id=\"button-box-4\"]"))).click()
        driver1.find_element(By.CSS_SELECTOR, "button[data-qa='btn-apply']").click()


        driver1.find_element(By.ID, "first-name-input").send_keys(profile['first_name'])
        driver1.find_element(By.ID, "last-name-input").send_keys(profile['last_name'])
        driver1.find_element(By.ID, "redesigned-input-email").send_keys(tempURL)
        driver1.find_element(By.ID, "password-input").send_keys(PASSWORD)
        ...
        def selectDropDown(dropdownId, itemSelector, country):
            driver1.execute_script(f'document.querySelector(\'#{dropdownId}\').click()')
            sleep(0.5)
            nations = driver1.find_elements(By.CSS_SELECTOR, itemSelector)

            if str(type(country)) == "<class 'int'>":
                driver1.execute_script(f'document.querySelectorAll("{itemSelector}")[{str(country)}].click()')
            else:
                for i in range(len(nations)):
                    try:
                        if nations[i].text.find(country) >= 0:
                            driver1.execute_script(f'document.querySelectorAll("{itemSelector}")[{str(i)}].click()')
                            break
                    except:
                        pass
        ...
        driver1.execute_script('document.querySelectorAll("span.up-checkbox-replacement-helper")[1].click()')
        sleep(1)
        driver1.execute_script('document.getElementById("button-submit-form").click()')
        sleep(9)
        def verifyEmail():
        ...
        if hasResume:
            ...
            WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "a[class=\"up-n-link\"]"))).click()

            # file_selector = WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[type=\"file\"]"))).click()
            ...
            def input_skills(skill):
                skillsInput = WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[aria-labelledby=\"skills-input\"]")))
            ...
            try:
                ...
                upload_photo_button = WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[aria-label=\"Upload photo\"]")))
                ...
                # WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[id=\"image-crop-17\"]"))).click()
                ...
                # WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[class=\"air3-typeahead-input-main air3-input\"]"))).send_keys(CITY_NAME)
            ...
            driver.get("https://www.upwork.com/nx/jobs/search/?q=web%20OR%20wordpress%20OR%20shopify%20OR%20ecommerce%20OR%20woocommerce%20OR%20wix%20OR%20webflow&sort=recency&t=0,1&amount=0-99,100-499,500-999,1000-4999,5000-&payment_verified=1")
            ...
            boost_button = WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[data-ev-label=\"boost_set_bid_amount\"]")))
            ...
            WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[type=\"checkbox\"]"))).click()
            WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class=\"up-btn up-btn-primary m-0 btn-primary\"]"))).click()
            WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "input[type=\"checkbox\"]"))).click()
            WebDriverWait(driver, 45).until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class=\"up-btn up-btn-primary m-0 btn-primary\"]"))).click()
